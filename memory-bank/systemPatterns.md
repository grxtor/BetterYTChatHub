...
- **Unified Settings Model**: Shared `AppSettings` type and `DEFAULT_APP_SETTINGS` live in `shared/settings.ts`, ensuring dashboard, settings, overlay, superchat, members, and backend all consume the same settings contract.
- **Settings Normalization + Sync Pattern**: Frontend settings flow runs through `normalizeSettings()`, `loadStoredSettings()`, `persistSettings()`, and `subscribeToSettingsChanges()`. This normalizes legacy values (notably old percentage-based overlay scales), writes to `localStorage`, broadcasts updates via `BroadcastChannel` plus a custom window event, and keeps overlay pages aligned with SSE-delivered backend updates.
- **Workspace Shell Pattern**: The operator dashboard now exposes app-level workspace controls (`workspaceFrame`, `dashboardDensity`, `dashboardPanelWidth`, `accentColor`, `showAmbientGlow`, `showBadges`, `showSelectionPreview`) through the shared settings model. These settings affect both the settings preview and the live dashboard shell so the UI behaves more like an Electron control room than a narrow website.
- **Dashboard Command Deck Layout**: Dashboard is organized as a command-deck shell with three summary metric cards, a searchable primary feed, and a configurable right rail for pinned selection preview plus revenue/community panels. The right rail width is driven by a CSS variable sourced from shared settings, keeping layout changes reactive without hardcoding widths in multiple components.
- **Deferred Feed Search Pattern**: Feed filtering uses React's `useDeferredValue()` so operator search remains responsive under heavier message throughput while still composing with author filtering and auto-scroll logic.
- **Tailwind + Legacy CSS Layering Rule**: When mixing Tailwind v4 utilities with handcrafted global CSS, legacy styles must live inside explicit cascade layers (`@layer base` / `@layer components`). Unlayered CSS will override Tailwind utilities such as spacing and auto margins, producing broken layouts even when the utility classes are present in the DOM.
